...
Keil 直接加载 APP AXF 时仍可设置断点并单步调试。正常上电复位后的 Bootloader 启动仍受现有升级有效状态约束；需要验证完整复位跳转时，应通过升级流程写入 APP，或使用明确的开发调试流程设置有效状态，不能在量产 Bootloader 中绕过中断升级保护。
